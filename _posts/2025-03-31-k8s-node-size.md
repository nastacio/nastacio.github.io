...
Kubernetes hides its resource waste in plain sight, under a cloak of abstracted layers and compartments. From physical limits [tracing their lineage to the early 19th century](https://en.wikipedia.org/wiki/19-inch_rack) to a [dizzying array of resource allocation techniques](https://dnastacio.medium.com/kubernetes-resources-1a1fa1e72dcf?source=friends_link&sk=0ae9728422d149d00e0761f438bbd019), it is virtually impossible for understand all nuances of how workloads are distributed across the environment.

There are three ways in IT to deal with things we don't understand, which are especially true of resource allocation in Kubernetes.

1. __Learn more about those things__. Learning more is the theme of this article. The article does not cover _everything_, but it covers the most common blind spots. Knowing what to avoid and why also helps one focus one's observations about the system.
2. __Reboot it__. Whether restarting a pod, draining a node, or restarting a compute instance, "reboot it" is the [Cargo Cult](https://lonesysadmin.net/2011/10/19/cargo-cult-system-administration/) variation of resource allocation. It starts with something resembling resourcefulness, such as _"I restarted the node, and the problem went away"_. To the trained eye, however, the "solution" is an illusion. Those same pods are reassigned to different nodes, which will cause the same problems a few hours later.
3. __Increase capacity__. Some problems may require more capacity, while others may just happen to be resolved with more capacity. For example, suppose a pod has an undeclared memory utilization of 32GB that keeps crashing the nodes where the pod is scheduled. A system administrator unaware of the [Kubernetes resource allocation model](https://kubernetes.io/docs/concepts/configuration/manage-resources-containers/) may decide to reallocate the entire node pool with an extra 32GB for each node.
...
Separating the unavoidable from the unnecessary is a rare and often undervalued skill. It is not until the organization's annual budget starts to hit millions of dollars that the demand for those skills comes to the forefront.

If you are reading this article, you likely are that "someone" trying to help the organization tame its budget.

Then you quickly realize making sure a fleet of Kubernetes clusters is right-sized and stays right-sized is not an easy challenge.
...
Limits lurking deep in that stack are a perennial source of seemingly mysterious messages indicating that pods cannot be scheduled on a lightly loaded node.
...
__Takeaway__: Read your cloud provider documents surrounding those three abstraction layers (Kubernetes, compute instance, and IaaS account.)
...
Many open-source and commercial tools can help you analyze the historical resource usage of workloads in a cluster. These tools can identify workloads that may be underperforming due to a lack of resources and, more importantly, those that can destabilize other workloads.

For larger environments, some of these tools can continuously monitor resource utilization, creating optimization plans for setting workload resource requests and limits. For smaller use cases, one can run a simple `kubectl` command like [this](https://stackoverflow.com/a/73190962/996026) or something more sophisticated, like Robusta's [Kubernetes Resource Recommender](https://github.com/robusta-dev/krr).
